// EXACT mirror of CURRENT workers-1.4.js handleIntelTV (onAirIds + full trendRecovery
// detail + intelComputeScore with lastEp). Confirm 207333 → ongoing, budget < 50.
...
// intelComputeScore (current worker: lastEp-aware)
...
async function main(){
  ...
  console.log("onTheAirCandidates:",onTheAirCandidates.length,"| trendRecovery:",trendRecovery.length,"| 207333 in trendRecovery:",trendRecovery.some(s=>s.id===207333));
  const trendHydrated=await epDates(trendRecovery);
  console.log("after trendRecovery detail budget:",budget,"| 207333 last_ep:",trendHydrated.find(s=>s.id===207333)?.last_episode_to_air?.air_date);
  ...
  console.log("selected:",selAll.length,"|",selAll.map(s=>s.name).join(" | "));
  ...
  console.log("*** BUDGET:",budget," (premieres detail extra ~0-8) ***");
}
